...
export default {
  ...
  methods: {
    ...
    submit(e){
      ...
      axios
        .post('http://localhost:3000/tugboat', data)
        .then(response => {
          if (response.status == 201 ){
            ...
            this.$store.state.perfil = "Tugboat";
            this.$router.push('/');
          }
        })
        .catch(error => {
    ...
    loadInfo(){
      ...
      this.title = this.$t("tugboat");
      this.subtitle = this.$t("informations");
      axios
        .get(`http://localhost:3000/tugboat?cnpj=${defaultCnpj}`)
        .then(response => {
    ...
    isUpdating(){
      if (this.$route.path.toLowerCase() == '/tugboat/update'){
        return true;
      }
      return false;
    }
