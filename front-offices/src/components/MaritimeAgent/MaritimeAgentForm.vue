...
export default {
  ...
  methods: {
    ...
    submit(e){
      ...
      axios
        .post('http://localhost:3000/maritime-agent', data)
        .then(response => {
          if (response.status == 201 ){
            ...
            this.$store.state.perfil = "MaritimeAgent";
            this.$router.push('/');
          }
        })
        .catch(error => {
    ...
    isUpdating(){
      if (this.$route.path.toLowerCase() == '/maritimeagent/update'){
        return true;
      }
      return false;
    }
