...
export default {
  ...
  methods: {
    ...
    submit(e){
      ...
      axios
        .post('http://localhost:3000/mooring-operators', data)
        .then(response => {
          if (response.status == 201 ){
            ...
            this.$store.state.perfil = "MooringOperators";
            this.$router.push('/');
          }
        })
        .catch(error => {
    ...
    loadInfo(){
      ...
      this.title = this.$t("mooringOperators");
      this.subtitle = this.$t("informations");
      axios
        .get(`http://localhost:3000/mooring-operators?cnpj=${defaultCnpj}`)
        .then(response => {
    ...
    isUpdating(){
      if (this.$route.path.toLowerCase() == '/mooringoperators/update'){
        return true;
      }
      return false;
    }
