      ...
      <v-card-text>
        <form>
          ...
          <v-col cols="12" lg="6">
            ...
            <br>
            <v-select v-model="$v.Selected.$model" :items="Items" :label="$t('shipLoad')" 
              class="pb-12" single-line 
              item-text="desc" item-value="type" required return-object
              :error-messages="shipLoadErrors" @input="$v.Selected.$touch()" 
            ></v-select>
          </v-col>
...
export default {
  ...
  data () {
    return {
      ...
      Selected: null,
      Items:[
        { type: 'Comidas', desc: 'Comidas'},
        { type: 'AlimentosFrescos', desc: 'Alimentos frescos ou secos'},
        { type: 'Bebidas', desc: 'Bebidas'},
        { type: 'Pecas', desc: 'Peças e ferramentas'},
        { type: 'Barcos', desc: 'Barcos, botes e equipamentos salva vidas'},
        { type: 'EquipCombate', desc: 'Equipamentos de combate a incendio'},
        { type: 'EquipSeguranca', desc: 'Equipamentos de segurança'},
        { type: 'Publicacoes', desc: 'Publicações náuticas'},
        { type: 'Produtos', desc: 'Produtos químicos (pode ser Eco-friendly)'},
        { type: 'Gases', desc: 'Gases (solda, refrigeração, etc.)'}
      ]
    }
  },
  validations () {
  ...
  computed: {
    ...
    shipLoadErrors () {
      ...
      !this.$v.Selected.required && errors.push(this.$t('text.form.errors.shipLoadRequired'))
      return errors
    },
  },
  methods: {
    ...
    submit(e){
      ...
      axios
        .post('http://localhost:3000/ship-supplier', data)
        .then(response => {
          if (response.status == 201 ){
            ...
            this.$store.state.perfil = "ShipSupplier";
            this.$router.push('/');
          }
        })
        .catch(error => {
    ...
    loadInfo(){
      ...
      this.title = this.$t("shipSupplier");
      this.subtitle = this.$t("informations");
      axios
        .get(`http://localhost:3000/ship-supplier?cnpj=${defaultCnpj}`)
        .then(response => {
    ...
    isUpdating(){
      if (this.$route.path.toLowerCase() == '/shipsupplier/update'){
        return true;
      }
      return false;
    }
